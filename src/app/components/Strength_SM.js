export default function Strengths_SM() {
  const strength = [
    {
      strength_icon: (
        <svg
    ...
    <div className="w-full bg-blue-950 py-12">
      <div className="container mx-auto grid grid-cols-1  gap-8 px-4">
        {strength.map((item, idx) => (
          <div
            key={idx}
            className="bg-slate-200 text-slate-900 p-6 rounded-lg shadow-lg transform transition-transform hover:scale-105"
            data-aos="fade-up"
          >
            <div className="flex flex-col items-center justify-center h-full">
              <div className="mb-4">{item.strength_icon}</div>
